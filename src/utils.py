...
def find_top_websites(data, url_column="url", top=10):
    """find the top websites that are shared in the channel
    args: data: the data that has the url column
          url_column: the column that has the url
          top: the number of top websites to return
    """
    if url_column in data.columns.values:
        websites = []
        for url in data[url_column]:
            if url != None:
                websites.append(url.split("/")[2])
        return Counter(websites).most_common(top)
    else:
        print(f"{url_column} not in data")


def find_high_traffic_websites(data, url_column="url", threshold=10):
    ...
    if url_column in data.columns.values:
        ...
        for url in data[url_column]:
            if url != None:
                websites.append(url.split("/")[2])
        return Counter(websites).most_common(threshold)

    else:
        print(f"{url_column} not in data")


def find_high_traffic_find_top_websites(data, top=10):
...
def plot_top_websites(data, url_column="url", top=10):

    websites = find_top_websites(data, url_column, top)
    if websites is not None:
        website_names = [website[0] for website in websites]
        website_counts = [website[1] for website in websites]

        plt.figure(figsize=(10, 5))
        plt.bar(website_names, website_counts)
        plt.xticks(rotation=90)
        plt.title(f"Top {top} websites shared in the channel")
        plt.show()
